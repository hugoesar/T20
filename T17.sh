...
check_disk() {
	...
	echo "ALERTS: Uso del disco por encima del $DISK_THRESHOLD%!"
	if [ $disk_usage -gt $DISK_THRESHOLD ]; then
		echo "ALERTA: Uso del disco por encima del $DISK_THRESHOLD%!"
	fi
}

#Función para verificar el rendimeinto del sistema con vmstat
check_vmstat() {
	vmstat 1 5
}

#Función para vferificar el rendimiento del disco con iostat
check_iostat() {
	iostat -dx 1 5
}
